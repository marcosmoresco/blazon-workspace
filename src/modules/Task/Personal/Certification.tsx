...
//queries
import { GET_CERTIFICATION_APPROVAL_TASKS } from "@modules/Task/queries";

const PersonalTasksCertification: FC<ListProps> = ({ dispatch, filtered = {} }) => {
  ...
  return (    
    <>
      {(data?.getCertificationApprovalTasks?.representation || []).length > 0 && (
        <Tasks list={data?.getCertificationApprovalTasks.representation || []} type="CERTIFICATION_TASK" subType="certification" size={size} filteredString={filteredString}/>
      )}
      {(data?.getCertificationApprovalTasks?.representation || []).length === 0 && (
