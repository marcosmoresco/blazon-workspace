...
const PersonalTasksRoleRight: FC<ListProps> = ({ dispatch, filtered = {}, checkAll = false, setCheckAll }) => {
  ...
  const { loading: loadingActions, data: dataActions,  refetch: refetchActions } = useQuery<{
    getActions: string[]
  }>(getAvailableActionsByType("roleRight"), {
    variables: {
      status: `["TODO"]`
    },  
  });

  const [assignToMe, {}] = useMutation(ASSIGN_TO_ME_ROLE_RIGHT_APPROVAL_TASK, { 
    refetchQueries: [
      {
  ...
  const [unassign, {}] = useMutation(UNASSIGN_ROLE_RIGHT_APPROVAL_TASK, { 
    refetchQueries: [
      {
        query: GET_ROLE_RIGHT_APPROVAL_TASKS,
        variables: {          
  ...
  const [forwardToUser, {}] = useMutation(FORWARD_TO_USER_ROLE_RIGHT_APPROVAL_TASK, { 
    refetchQueries: [
      {
        query: GET_ROLE_RIGHT_APPROVAL_TASKS,
        variables: {          
  ...
  const [forwardToQueue, {}] = useMutation(FORWARD_TO_QUEUE_ROLE_RIGHT_APPROVAL_TASK, { 
    refetchQueries: [
      {
        query: GET_ROLE_RIGHT_APPROVAL_TASKS,
        variables: {          
  ...
  useEffect(() => {
    ...
    if(checkAll !== checkedAll) {
      ...
      if(checkAll) {
        (data?.getRoleRightApprovalTasks?.representation || [])
  ...
  const checkAssignActions = (newChecked: number[]) => {
    ...
    newChecked.forEach((n) => {
      const ts = (data?.getRoleRightApprovalTasks?.representation || []).filter((t: any) => t.identifier === n);
...
export default connect(mapStateToProps)(PersonalTasksRoleRight);
