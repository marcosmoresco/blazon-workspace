...
//queries
import { GET_SOD_APPROVAL_TASKS } from "@modules/Task/queries";
  ...
  return (    
    <>
      {(data?.getSoDApprovalTasks?.representation || []).length > 0 && (
        <Tasks list={data?.getSoDApprovalTasks.representation || []} type="SOD_TASK" subType="sod" size={size} filteredString={filteredString}/>
      )}
      {(data?.getSoDApprovalTasks?.representation || []).length === 0 && (
