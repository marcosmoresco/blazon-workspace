...
//queries
import { GET_PROVISIONING_TASKS } from "@modules/Task/queries";
  ...
  return (   
    <>
      {(data?.getProvisioningTasks?.representation || []).length > 0 && (
        <Tasks list={data?.getProvisioningTasks.representation || []} type="PROVISIONING_TASK" subType="provisioning" size={size} filteredString={filteredString}/>
      )}
      {(data?.getProvisioningTasks?.representation || []).length === 0 && (
