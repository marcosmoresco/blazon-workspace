...
const PersonalTasksProvisioning: FC<ListProps> = ({ dispatch, filtered = {}, checkAll = false, setCheckAll, orderBy = "createdDate:desc" }) => {
  ...
  const { loading: loadingActions, data: dataActions,  refetch: refetchActions } = useQuery<{
    getActions: string[]
  }>(getAvailableActionsByType("provisioning"), {
    variables: {
      status: `["TODO"]`
    },  
  });

  const [assignToMe, {}] = useMutation(ASSIGN_TO_ME_PROVISIONING_TASK, { 
    refetchQueries: [
      {
  ...
  const [unassign, {}] = useMutation(UNASSIGN_PROVISIONING_TASK, { 
    refetchQueries: [
      {
        query: GET_PROVISIONING_TASKS,
        variables: {          
  ...
  const [forwardToUser, {}] = useMutation(FORWARD_TO_USER_PROVISIONING_TASK, { 
    refetchQueries: [
      {
        query: GET_PROVISIONING_TASKS,
        variables: {          
  ...
  const [forwardToQueue, {}] = useMutation(FORWARD_TO_QUEUE_PROVISIONING_TASK, { 
    refetchQueries: [
      {
        query: GET_PROVISIONING_TASKS,
        variables: {          
  ...
  useEffect(() => {
    ...
    if(checkAll !== checkedAll) {
      ...
      if(checkAll) {
        (data?.getProvisioningTasks?.representation || [])
  ...
  const checkAssignActions = (newChecked: number[]) => {
    ...
    newChecked.forEach((n) => {
      const ts = (data?.getProvisioningTasks?.representation || []).filter((t: any) => t.identifier === n);
      if((ts || []).length) {
