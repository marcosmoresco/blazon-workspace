...
export default function CustomizedAutocomplete(props:any) {
  ...
  React.useEffect(() => {  
    ...
    if(async && autoCompleteRef && autoCompleteRef.current && !autoCompleteRef?.current?.isLoading && ((!options.length && !firstSearch) || autoCompleteRef?.current?.changed)) {
      setLoading(true)
  ...
  const filterOptions = async ? createFilterOptions({   
  ...
  return (
    <Autocomplete
      ...
      options={options || []}
      getOptionLabel={(option:any) => (getOptionLabel ? getOptionLabel(option) : (label ? option[label] : option.title))}              
