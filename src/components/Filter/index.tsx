...
class Filter extends Component<FilterPropsType, FilterStateType> {
    ...
    if (selectedFilter) {
      if (selectedFilter.type === "date" && type) {
        ...
        if (selectedFilter.list) {
          const filtered = selectedFilter.list.filter(
            (s: FilterType) => s.name === type
          );
  ...
  addFilter(e: any, _filter?: FilterType) {
    ...
    if (_selectedFilter) {
      const filterList = filters.filter(
        (f: FilterType) => f.name === _selectedFilter.name
      );
      ...
      this.setState({
        ...
        selectedIndex: undefined,
        filtered:
          filtered.indexOf(_selectedFilter.name) === -1
            ? [...filtered, _selectedFilter.name]
            : [...filtered],
      });
        ...
        filters.forEach((f: FilterType) => {
          if (
            ...
            if (f.type === "list" && f.bind && f.values.length > 10) {
              ...
              _filters[f.bindValue ? f.bindValue : f.name] =
                (f.formatValue && f.formatValue(f.value)) || f.value;
            }
          }
        });
        onChange(_filters);
  ...
  removeFilter(event: any, item: FilterType) {
    ...
    this.setState({
      selectedFilter:
        selectedFilter && selectedFilter.name === item.name
          ? undefined
          : selectedFilter,
      selectedIndex:
        selectedFilter && selectedFilter.name === item.name
  ...
  getFilterComponent() {
    ...
    let filter =
      (selectedIndex !== undefined && filters[selectedIndex]) || null;
    let component = null;
      ...
      if (["text", "number"].indexOf(filter.type) > -1) {
        ...
        if (filter.values && filter.values.length) {
              ...
              <div>
                ...
                <Autocomplete
                  ...
                  onChange={(event: any, value: any) =>
                    this.handleChangeComponent(value)
                  }
                />
              </div>
            );
          ...
          if (filter.startAdornment) {
            inputprops.startAdornment = filter.startAdornment(
              selectedFilter.value
            );
          }
          ...
          component = (
            <div>
              ...
              <Autocomplete
                ...
                async={(query: any, callback: any) =>
                  (filter && filter.async(dispatch, query, callback)) || null
                }
                filterSelectedOptions
                label={filter.view}
                value={selectedFilter.value || { [filter.view]: "" }}
        ...
        component = (
          <div className="Date-filter">
            <DatePicker
              ...
              onChange={(date: any) =>
                this.handleChangeComponent(date, "start")
              }
            />
            <DatePicker
              isTime
        ...
        component = selectedFilter.list.map((f: FilterType) => {
          return (
            <div key={`filter-group-${f.name}`}>
              <TextField
                ...
                value={f.value || ""}
                onChange={(event: any) =>
                  this.handleChangeComponent(event.target.value, f.name)
                }
                placeholder={
                  f.type === "text" ? "Enter the text" : "Enter the number"
                }
              />
            </div>
          );
    ...
    const enableClick =
      ...
      ((selectedFilter.type === "date" &&
        ...
        (selectedFilter.type !== "date" && selectedFilter.value));
      ...
      (component && (
        <React.Fragment>
          <div
            ...
            onKeyPress={(event) => this.handleKeyDown(event, enableClick)}
          >
            <div className="Filter-component">{component}</div>
            <div
              className="Add-icon"
      ...
      filters.forEach((f: FilterType) => {
        if (
          ...
          (!filterText ||
            f.label
              .toLocaleUpperCase()
              .indexOf(filterText.toLocaleUpperCase()) > -1)
        ) {
          list.push(f);
        }
      });
                ...
                <div className={classes.content}>
                        ...
                        {filters.map((item: any, index: any) =>
                          filtered.indexOf(item.name) > -1 ? (
                            <div
                              ...
                              onClick={() =>
                                this.handleSelectFilter(index, item)
                              }
                            >
                              <Image
                                alt="X"
                  ...
                  <div
                    className={classNames({
                      "Filters-list": filtered && !!filtered.length,
                    })}
                  >
                    <label className={classes.title}>
                      <FormattedMessage id="filters" />
                    </label>
                    <div>{this.getFilterComponent()}</div>
                    <div className={classes.chipsContent}>
                      {filters.map((filter: any, index: number) =>
                        ...
                        (!filterText ||
                          ...
                          <div
                            ...
                            onClick={() =>
                              this.handleSelectFilter(index, filter)
                            }
                          >
                            <label>{filter.label}</label>
                          </div>
...
export default connect(mapStateToProps)(
  withStyles(useStyles)(injectIntl(Filter))
);
